...
class StrictMatcherTest {
    ...
    void anIdenticalRequestFingerprintHits() {
        MatchResult result = StrictMatcher.match(REQUEST, List.of(cassette("issue-routing", REQUEST)));
    ...
    void aDifferentRequestFingerprintMissesAndCarriesTheReplayKey() {
        MatchResult result = StrictMatcher.match(OTHER, List.of(cassette("issue-routing", REQUEST)));

        assertThat(result).isEqualTo(new MatchResult.Miss(OTHER));
    }
    ...
    void neitherStateNorContractAloneCanRescueAMiss() {
        ...
        assertThat(StrictMatcher.match(REQUEST, List.of(sameContractOtherRequest)))
                .isInstanceOf(MatchResult.Miss.class);
    }

    @Test
    void noCandidatesIsAMiss() {
        assertThat(StrictMatcher.match(REQUEST, List.of())).isEqualTo(new MatchResult.Miss(REQUEST));
    }

    @Test
    void theFirstCandidateInOrderWins() {
        // 装载顺序（按 cassette 名称排序）就是优先级，于是"第一个命中者胜出"是确定的。
        MatchResult result = StrictMatcher.match(REQUEST,
